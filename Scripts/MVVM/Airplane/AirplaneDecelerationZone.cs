...
{
    public sealed class AirplaneDecelerationZone : IDisposable
    {
        public enum EState
        {
            NotInited,
        ...
        public AirplaneDecelerationZone( DecelerationZoneSerializedBGConfig zone )
        {
            ...
            if ( _lerpEnterDuration <= 0f )
                throw new ArgumentException( "lerpEnterDuration passed has invalid value of zero or is negative!" );
            if ( _lerpExitDuration <= 0f )
                throw new ArgumentException( "lerpExitDuration passed has invalid value of zero or is negative!" );
        }

        public void Dispose()
        {
            if ( State == EState.Disposed )
                throw new GameException( "Deceleration zone was already disposed, check for duplication code" );
        ...
        public void SetSimulationParameters( bool isActive )
        {
            switch ( State )
            {
                ...
                {
                    if ( !isActive )
                        throw new GameException( "Airplane deceleration zone not inited, you should not try to deactivate it!" );
                ...
                case EState.Decreasing:
                {
                    if ( isActive )
                        throw new GameException(
                            "Airplane somehow enters same deceleration zone, you should not apply remaining speed up, just drop it"
                        );
                ...
                case EState.Lost:
                {
                    if ( isActive )
                        throw new GameException(
                            "Trying to activate already lost effect of deceleration zone, probably your data not flushed out properly"
                        );
                ...
                {
                    throw new GameException( "Deceleration zone is disposed, but you are trying to set simulation parameters for it" );
                }
                default:
                {
        ...
        public void Simulate( float deltaTime )
        {
            switch ( State )
            {
                case EState.NotInited:
                {
                    throw new GameException( "Trying to simulate not initialized airplane deceleration zone" );
                }
                case EState.Disposed:
                {
                    throw new GameException( "Can't simulate disposed deceleration zone!" );
                }
                case EState.Started:
                case EState.Increasing:
                {
        ...
        public bool IsSameZone( DecelerationZoneSerializedBGConfig zonePart )
        {
